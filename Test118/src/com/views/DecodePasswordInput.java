		...
		nextButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (!decodePassword.equals("")) {
					...
					mpDialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);// 设置风格为圆形进度
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {// handler接收到消息后就会执行此方法
			...
			case 0:
				...
				String fileName = MainActivity.CONFIG_PATH + "content.txt";// 文件路径
				String content = "";
				try {
					...
					fin.read(buffer);
					content = EncodingUtils.getString(buffer, "GBK");// //依Y.txt的编码类型选择合适的编码，如果不调整会乱码
					fin.close();// 关闭资源
